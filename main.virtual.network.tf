...
resource "azapi_resource" "vnet" {
  ...
  lifecycle {
    ...
    precondition {
      condition     = var.existing_vnet == null ? (var.resource_group_name != null && var.location != null && var.name != null && var.address_space != null) : true
      error_message = "`var.resource_group_name`, `var.location`, `var.name` and `var.address_space` must be specified unless `var.existing_vnet` is supplied."
    }
  }
}
