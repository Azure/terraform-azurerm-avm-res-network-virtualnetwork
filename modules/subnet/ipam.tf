resource "azapi_resource" "subnet_ipam" {
  count = local.ipam_managed_enabled ? 1 : 0
...
# Alternate IPAM subnet resource used when var.ignore_route_table_changes = true.
# Identical body to azapi_resource.subnet_ipam, but adds lifecycle.ignore_changes for
# body.properties.routeTable. See the comment on azapi_resource.subnet_ignore_route_table
# in main.tf for the rationale and the static-literal constraint.
resource "azapi_resource" "subnet_ipam_ignore_route_table" {
  count = local.ipam_ignore_rt_enabled ? 1 : 0
