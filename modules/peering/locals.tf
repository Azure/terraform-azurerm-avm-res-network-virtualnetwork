locals {
  ...
  output_peering_sync_status         = local.is_full_peering ? azapi_resource.this[0].output.properties.peeringSyncLevel : (local.is_address_space_peering ? azapi_resource.address_space_peering[0].output.properties.peeringSyncLevel : azapi_resource.subnet_peering[0].output.properties.peeringSyncLevel)
  ...
  output_reverse_peering_sync_status = var.create_reverse_peering ? (var.reverse_peer_complete_vnets ? azapi_resource.reverse[0].output.properties.peeringSyncLevel : (local.is_reverse_address_space_peering ? azapi_resource.reverse_address_space_peering[0].output.properties.peeringSyncLevel : azapi_resource.reverse_subnet_peering[0].output.properties.peeringSyncLevel)) : null
...
locals {
  sync_remote_address_space_query_parameter = var.sync_remote_address_space ? {
