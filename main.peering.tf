resource "azapi_resource" "vnet_peering" {
  for_each = var.use_existing_virtual_network ? {} : var.peerings
...
resource "azapi_resource" "reverse_vnet_peering" {
  for_each = var.use_existing_virtual_network ? {} : { for k, v in var.peerings : k => v if v.create_reverse_peering }
